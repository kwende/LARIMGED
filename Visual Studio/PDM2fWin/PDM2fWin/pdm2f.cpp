...
/*    Version 3.06 - 12/21/2011  -  data file version      */
...
char base_directory[1024]; 			   /*  the base directory path for all output files */
...
int pdm2_2(char* file, char* outputDirectory)
{
	if(!read_data_file( file, 1 )) 
	{
		return -1;  
	}
	else
	{
		int outputDirectoryPathLength = strlen(outputDirectory); 
		if(outputDirectoryPathLength > sizeof(base_directory)-1)
		{
			printf("outputDirectory length is %d\n", sizeof(base_directory)); 
			return -1; 
		}
		else if(outputDirectory[outputDirectoryPathLength-1] != '\\')
		{
		...
		return pdm2( Numdat, times, mags, sigs  );
	}
}
...

{
	...
	char *stp = NULL;
	FILE *fo = NULL, *fp = NULL, *fp2 = NULL, *fb = NULL;
	...
	if( do_beta_scan ) {
		...
		char szFullOutputPath[1024]; 
		strcpy(szFullOutputPath, base_directory); 
		strcat(szFullOutputPath, "beta_scan.csv"); 

		fb = fopen( szFullOutputPath, "w" );
	...
	for( kk = 1; kk <= nb; kk++ ) {
		...
		char szFullOutputPath[1024]; 
		strcpy(szFullOutputPath, base_directory); 
		strcat(szFullOutputPath,"data.csv"); 

		fp = fopen( szFullOutputPath, "w" );
		if(fp)
		{
			if( invert_curve ) {
		...
		if( do_dist ) {
			...
			char szFullOutputPath[1024]; 
			strcpy(szFullOutputPath, base_directory); 
			strcat(szFullOutputPath, "sig.csv"); 
		...
		char szFullOutputPath2[1024]; 
		strcpy(szFullOutputPath2, base_directory); 
		strcat(szFullOutputPath2, "pdmplot.csv"); 
		...
		char szFullOutputPath3[1024], szFullOutputPath4[1024]; 
		strcpy(szFullOutputPath3, base_directory);
		strcpy(szFullOutputPath4, base_directory);
		strcat(szFullOutputPath3, "pdmplot.csv"); 
		strcat(szFullOutputPath4, "residuals.csv"); 

		fp = fopen(szFullOutputPath3 , "w" );
		fp2 = fopen(szFullOutputPath4 , "w" );
		if(fp && fp2)
		{
		...
		if( do_dist ) {
			...
			char szFullOutputPath[1024]; 
			strcpy(szFullOutputPath, base_directory); 
			strcat(szFullOutputPath, "theta_dist.csv"); 
